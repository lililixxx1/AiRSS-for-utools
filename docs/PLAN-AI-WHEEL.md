...
6. **v1.7 加第四项「文内搜索」（2026-09-12 用户需求，PLAN-WHEEL-FIND，实施后追加）**：轮盘从三项扩为四项（摘要/翻译/目录/文内搜索），文内搜索项非 AI 功能——不受 aiEnabled 置灰、无 loading/done 态，AI 关时恒可点。几何随项数重排：R 92→110、角度 -100/-135/-170 → **30° 等距四分之一弧 -90/-120/-150/-180**（摘要(0,-110)/翻译(-55,-95)/目录(-95,-55)/文内搜索(-110,0)，错峰 0/45/90/135ms），相邻弦长 2·110·sin15°≈57px（≥原 55px 密度不降）；§1.2 热区推导更新为 **R110+项半径22+缓冲8=140**（替代旧「≥100/≥101 取 120」口径），inHotRect 左/上扩 120→140。点击语义新增：文内搜索=开 ReaderFind（恒可用，顺关轨内列表搜索面板——两搜索面双向互斥）。容器 menu 名与球 aria/title 同步四项措辞。
