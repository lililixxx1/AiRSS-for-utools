...
| 分离独立窗口 | 1200×800（自适应下限 960×600） | 三栏：侧栏 280 + 列表 400（360–440 弹性）+ 阅读 ≥440 |

- 基准网格 4px；所有间距、行高、圆角均为 4 的倍数（圆角 6 例外，用于 12px 高微型元素）。
- 内容区内边距 16px；卡片流为单列（卡片视图）或满宽行（列表视图），无多列网格。
- 800px 以下（不可能出现于小窗，仅防御分离窗极窄）：列表栏收缩至 360，阅读栏保持 ≥440。
...
- ResizeObserver：<960 时列表栏压到 360，仍三栏；阅读栏永远可见。
